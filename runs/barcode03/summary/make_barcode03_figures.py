...
BASE = "results/barcode03/summary"
...
def read_two_col_tsv(path, col1, col2):
    ...
    # If any weird whitespace, normalize:
...
# ---------------------------
df_mol = read_two_col_tsv(
    os.path.join(BASE, "amr_by_molecule_type.tsv"),
    "molecule_type", "count"
).sort_values("count", ascending=False)

fig, ax = plt.subplots()
ax.bar(df_mol["molecule_type"], df_mol["count"])
ax.set_title("Barcode03: AMR hits by molecule type")
ax.set_xlabel("Molecule type")
ax.set_ylabel("Number of AMR hits")
save_fig(fig, "barcode03_amr_by_molecule_type")

# ---------------------------
# Figure 2: AMR by drug class (horizontal bar)
# ---------------------------
df_dc = read_two_col_tsv(
    os.path.join(BASE, "amr_by_drug_class.tsv"),
    "drug_class", "count"
).sort_values("count", ascending=True)

fig, ax = plt.subplots(figsize=(8, max(4, 0.35 * len(df_dc))))
ax.barh(df_dc["drug_class"], df_dc["count"])
ax.set_title("Barcode03: AMR hits by drug class")
ax.set_xlabel("Count")
ax.set_ylabel("Drug class")
save_fig(fig, "barcode03_amr_by_drug_class")

# ---------------------------
# Figure 3: AMR by resistance mechanism (horizontal bar)
# ---------------------------
df_mech = read_two_col_tsv(
    os.path.join(BASE, "amr_by_mechanism.tsv"),
    "mechanism", "count"
).sort_values("count", ascending=True)

fig, ax = plt.subplots(figsize=(8, max(4, 0.35 * len(df_mech))))
ax.barh(df_mech["mechanism"], df_mech["count"])
ax.set_title("Barcode03: AMR hits by resistance mechanism")
ax.set_xlabel("Count")
ax.set_ylabel("Resistance mechanism")
save_fig(fig, "barcode03_amr_by_mechanism")
